...
func DownloadVideoCommand(ctx context.Context, b *bot.Bot, update *models.Update, args []string) error {
	...
	go func() {
		...
		fileName, err := utils.DownloadVideo(ctx, b, msg, video)
		if err != nil {
			resultChan <- result{Err: err}
		}
	...
	return uploadVideo(ctx, b, msg, res)
}

func uploadVideo(ctx context.Context, b *bot.Bot, msg *models.Message, res result) error {
	...
	params := &bot.SendVideoParams{
		ChatID:    msg.Chat.ID,
		Video:     &models.InputFileUpload{Filename: res.Video.Title + ".mp4", Data: file2},
